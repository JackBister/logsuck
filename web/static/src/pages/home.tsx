        ...
        <main role="main" class="container-fluid">
            ...
            >
              ...
              <div class="input-group mb-3">
                <input
                  ...
                  type="text"
                  class="form-control"
                  onInput={(evt) => this.onSearchChanged(evt)}
                  value={this.state.searchString}
                />
                <div class="input-group-append">
                  <TimeSelect
                    onTimeSelected={(ts) => this.setState({ selectedTime: ts })}
                  />
                  <button
                    disabled={this.state.state === HomeState.WAITING_FOR_SEARCH}
                    type="submit"
          ...
          <div class="result-container">
            ...
            {(this.state.state === HomeState.WAITING_FOR_SEARCH ||
              (this.state.state === HomeState.SEARCHED_POLLING &&
                this.state.searchResult.length === 0)) && (
              <div>Loading... There should be a spinner here!</div>
            )}
            {((this.state.state === HomeState.SEARCHED_POLLING &&
              this.state.searchResult.length > 0) ||
              this.state.state === HomeState.SEARCHED_POLLING_FINISHED) && (
              <div>
                {this.state.searchResult.length === 0 && (
                      ...
                      <div class="card mb-3 mb-xl-0">
                            ...
                            <tbody>
                              {Object.keys(this.state.topFields).map((k) => (
                                <tr
                                  ...
                                  onClick={(evt) => {
                                    evt.stopPropagation();
                                    this.onFieldClicked(k);
                                  }}
                                  class="test field-row"
                                >
                                  <td>{k}</td>
                                  {(this.state.state ===
                                    HomeState.SEARCHED_POLLING ||
                                    this.state.state ===
                                      HomeState.SEARCHED_POLLING_FINISHED) && (
                                    <td style={{ textAlign: "right" }}>
                                      {this.state.topFields[k]}
                                    </td>
                                  )}
                                </tr>
                              ))}
                        ...
                        )}
                        {
                          ...
                          >
                            <table class="table table-sm table-hover">
                              <tbody>
                                {this.state.selectedField?.topValues.map(
                                  (f) => (
                                    <tr
                                      key={f.value}
                                      onClick={() =>
                                        this.onFieldValueClicked(f.value)
                                      }
                                      style={{ cursor: "pointer" }}
                                    >
                                      <td class="field-value">{f.value}</td>
                                      <td
                                        class="field-value-count"
                                        style={{ textAlign: "right" }}
                                      >
                                        {f.count}
                                      </td>
                                      <td
                                        class="field-value-percentage"
                                        style={{ textAlign: "right" }}
                                      >
